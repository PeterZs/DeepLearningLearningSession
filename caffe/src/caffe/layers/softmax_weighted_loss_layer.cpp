...
namespace caffe {
  ...
  void SoftmaxWithWeightedLossLayer<Dtype>::LayerSetUp(
    ...
    if (this->layer_param_.softmax_param().has_weight_source()) {
      ...
      while (infile >> tmp_val) {
        ...
        label_weights_.push_back(tmp_val);
      }
      infile.close();
    
      CHECK_EQ(label_weights_.size(), prob_.channels());
    } else {
      LOG(INFO) << "Weight_Loss file is not provided. Assign all one to it.";
      label_weights_.assign(prob_.channels(), 1.0);
    }
  }
  ...
  void SoftmaxWithWeightedLossLayer<Dtype>::Forward_cpu(
	...
	const Dtype w = label_weights_[label_value];
