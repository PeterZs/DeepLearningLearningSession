...
int main(int argc, char *argv[])
{
  ...
  for (int image_index = 1; image_index <= FLAGS_num_images; image_index++) {
    string image_filename_1 = FLAGS_image_path_1 + "/" + to_string(image_index) + ".jpg";
    string image_filename_2 = FLAGS_image_path_2 + "/" + to_string(image_index) + ".jpg";
    if (invalid_images_1.count(image_index) == 0 && invalid_images_2.count(image_index) == 0) {
      if (rand() % 10000 < FLAGS_testing_image_ratio * 10000)
        test_list_out_str << image_filename_1 << " " << image_filename_2 << endl;
      else
        train_list_out_str << image_filename_1 << " " << image_filename_2 << endl;
    }
  }
